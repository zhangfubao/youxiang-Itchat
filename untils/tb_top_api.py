...
class TbApiClient(object):
    ...
    def taobao_tbk_dg_material_recommend(self, material_id: str, relation_id="", device_type="", device_encrypt="",
                                         device_value="",
        ...
        # 原因是：1. 不同 material_id 得到的数据不一，且刷新周期不一
        #                    2. 微信发送不可太频繁，我仅是怕被封，决定取很小一部分数据
        page_no = str(random.randint(0, 200))
    ...
    def taobao_tbk_dg_material_optional_upgrade(self, material_id: str, start_dsr="2000", start_tk_rate="0300",
                                                end_tk_rate="", start_price="",
                                                end_price="", is_overseas="", is_tmall="", sort="tk_rate_des",
                                                itemloc="", cat="", q="", has_coupon="",
                                                ip="", need_free_shipment="true", need_prepay="",
                                                include_pay_rate_30="", include_good_rate="",
                                                include_rfd_rate="", npx_level="", device_encrypt="", device_value="",
                                                device_type="",
                                                special_id="", relation_id="", get_topn_rate="", biz_scene_id="",
                                                promotion_type="",
                                                mgc_start_time="", mgc_end_time="", mgc_status="", ucrowd_id="",
                                                ucrowd_rank_items=""):
        '''
        通用物料推荐，传入官方公布的物料id，可获取指定物料
        淘宝接口文档：
        ...
        postparm = {
            ...
            'q': q,
            # (必填)商品筛选-查询词；注意：用标题精准搜索时，若无消费者比价场景ID2权限，当搜索结果只有一个商品时则出参不再提供商品推广链接和商品id字段，若搜索结果仍有多个商品，则正常出参。同时无消费者比价场景ID2权限，q参数也不再支持入参淘宝复制的商品链接进行搜索查询，仅支持入参含新商品id的淘宝客推广链接如uland链接进行搜索查询(场景id使用说明参考《淘宝客新商品ID升级》白皮书：https://www.yuque.com/taobaolianmengguanfangxiaoer/zmig94/tfyt0pahmlpzu2ud)
