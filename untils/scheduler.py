...
def jd_job_task(scheduler):
    ...
    if conf.get('app_key') == '' or conf.get('app_secret') == '' or conf.get('site_id') == '' or conf.get(
            'suo_im') == '':
        return
...
def scheduler_listener(event):
    ...
    if event.exception:
        print(
            f'''Error: JOB_ID: {event.job_id}, 运行时间：{(event.scheduled_run_time).strftime("%Y-%m-%d %H:%M:%S.%f")[0:19]}, 任务出错了！所有程序暂停！''')
        # 别闹，不会暂停，就是一轮错误罢了。
    else:
        print(
            f'''Success: JOB_ID: {event.job_id}, 运行时间：{(event.scheduled_run_time).strftime("%Y-%m-%d %H:%M:%S.%f")[
                                                           :-3]}, 任务运行成功，继续运行...''')
